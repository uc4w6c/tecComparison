...
// 年数から支払回数を計算
export const repaymentMouthCountCalc = (repaymentPeriod: number) : number => {
    return repaymentPeriod * 12;
};

// 現在の残高と年利から今月の支払い利息を計算
export const calcMonthInterestCalc = (balance: number, interestRate: number) : number => {
    return roundDown(balance * mouthRateCalc(interestRate), 0);
};

// 毎月の一定返済金額を計算する
export const fixedRepaymentAmountCalc = (balance: number, interestRate: number, repaymentPeriod:number) : number => {
